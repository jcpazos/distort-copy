...
<html>
  <head>
    ...
    <script type="text/javascript" charset="utf-8" src="base32k.min.js"></script>
  </head>
  ...
  <body>
    ...
    <script>
      ...
      function iterate(test, k, step, max) {
        // store
        try {
          sessionStorage.setItem("data", test.enc(test.data.slice(0, k)));
        } catch (error) {
          test.done(true);
          return false;
        }
        // verify
        var data = test.dec(sessionStorage.getItem("data"));
      ...
      function run(test) {
        iterate(test, 4 * 1024, 4 * 1024, 10 * 1024 * 1024);
      }
